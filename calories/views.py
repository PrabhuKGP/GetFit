...
def HomePageView(request):
	...
	calories = Profile.objects.filter(user=request.user).last()
...
def select_food(request):
	...
	food_items = Food.objects.filter(user=request.user)
...
def ProfilePage(request):
	...
	records=Profile.objects.filter(date__gte=some_day_last_week,date__lt=timezone.now().date(),user=request.user)
